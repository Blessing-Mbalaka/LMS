...
def extract_full_docx_structure(docx_file):
    """
    Reads a .docx file and returns a list of blocks in their original order.
    Each block is one of:
      - {"type": "question_header", "text": "1.2 …"}
      - {"type": "case_study",      "text": "…"}
      - {"type": "paragraph",       "text": "…"}
      - {"type": "table",           "rows": [[...], ...]}
    Now groups any paragraphs following a Case Study header as case-study until the next question header.
    """
    docx_file.seek(0)
    try:
        doc = Document(io.BytesIO(docx_file.read()))
    ...
    # Regex: question numbers and Case Study headers
    qn_rx = re.compile(r'^(\d+(?:\.\d+)+)\b')
    cs_start_rx = re.compile(r'(?i)^Case Study\s*:?\s*')
    ...
    for block in doc.element.body:
        ...
        if block.tag.endswith('}p'):
            ...
            # 2) Case Study start
            if cs_start_rx.match(text):
                in_cs = True
                cs_body = cs_start_rx.sub('', text).strip()
                blocks.append({"type": "case_study", "text": cs_body})
                continue

            # 3) Continuation of case study
            if in_cs:
                blocks.append({"type": "case_study", "text": text})
                continue
